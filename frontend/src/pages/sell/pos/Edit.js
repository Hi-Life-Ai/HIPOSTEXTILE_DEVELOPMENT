...
const Poscreate = () => {
    ...
    const { isUserRoleAccess, allLocations, allProducts,allPos,allTaxrates } = useContext(UserRoleAccessContext);
    ...
    const [posAdd, setPosAdd] = useState({
        discountvalue:"",businesslocation:"",referenceno: "", customer: "Walk-in-customer", date: "", counter: "", location: "", contactnumber: "", contactid: "", creditlimit: "",
    ...
    const [isDraftTrue, setIsDraftTrue] = useState(false)
    const [isQuotationTrue, setIsQuotationTrue] = useState(false)
  ...
  today = yyyy + '-' + mm + '-' + dd;
    ...
    const handleClickOpenpay = () => { {
        if(tableData.length == 0){
            setShowAlert("Please select any one product detail!");
            alertOpen();
        }else{
            setIsPay(true);
        }
    ...
    const fetchDraftid = async () => {
        try {
            let req = await axios.get(`${SERVICE.DRAFT_SINGLE}/${id}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            });
            ...
            let selectlocation = allLocations.filter((data, index) => {
                return data.locationid == req?.data?.sdraft?.businesslocation
            })
            setIsLocation(selectlocation[0]);

	       setIsDraftTrue(true);
            setPosAdd(req?.data?.sdraft)
            setTableData(req?.data?.sdraft?.goods);
            await fetchCustomer(req?.data?.sdraft?.customerledgeid)
        } catch (err) {
            const messages = err?.response?.data?.message;
    ...
    const fetchQuotid = async () => {
        try {
            ...
            let selectlocation = allLocations.length > 0 && allLocations.filter((data, index) => {
                return data.locationid == req?.data?.squotation?.businesslocation
            })
            setIsLocation(selectlocation[0]);	
	        setIsQuotationTrue(true);
            setPosAdd(req?.data?.squotation)
            setTableData(req?.data?.squotation?.goods);
            await fetchCustomer(req?.data?.squotation?.customerledgeid)
        } catch (err) {
            const messages = err?.response?.data?.message;
            console.log(messages);
        }
    ...
    const fetchDiscounts = async () => {
        try {
            ...
            let result = res.data.discounts.length > 0 && res.data.discounts.filter((data, index) => {
   ...
   const fetchLocation = async () => {
    try {
        ...
        var setngresult = allLocations?.filter((item) => {
            if (item.locationid == setngs.businesslocation) {
                return item
            }
        })
        ...
        if (isAlreadyAdded) {
            ...
            setTableData((tableData) => {
                return [...tableData, {
                    ...
                    sellingexcludevalue: e.sellingpricetax == 'Exclusive' ? e.sellingprice : ((Number(e.sellingprice) * (Number(e.applicabletax == "" || e.applicabletax == "None" ? 0 : getTaxRateData[0]?.taxrate) / 100) - Number(e.sellingprice))),
                    sellingincludevalue: e.sellingpricetax == 'Inclusive' ? e.sellingprice : ((Number(e.sellingprice) * (Number(e.applicabletax == "" || e.applicabletax == "None" ? 0 : getTaxRateData[0]?.taxrate) / 100)) + Number(e.sellingprice)),
        ...
        //pos auto id
    {
        ...
        try {
            ...
            handleprint();
	 	if(isDraftTrue){
	    	let response = await axios.delete(`${SERVICE.DRAFT_SINGLE}/${draftid}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            	});
	    	}else if(isQuotationTrue){
    ...
    const sendDraft = async () => {
        ...
        try {
            let PRODUCT_REQ = await axios.put(`${SERVICE.DRAFT_SINGLE}/${id}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                customer: String(posAdd.customer),
    ...
    const totalTaxValCal = () => {
        ...
        if (tableData?.length > 0) {
            tableData?.forEach((value) => {
                totaltaxvalue += Math.abs(Number(value.discountvalue)) * value.quantity
            })
        ...
        >
            ...
            <form >
                ...
                <Box sx={userStyle.container}>
                    <Grid container spacing={1} sx={userStyle.poscontainer} >
                        ...
                        <Grid item lg={2} md={5} sm={4} xs={10} sx={{ marginTop: "6px" }}>
                        ...
                        <Grid item lg={1} md={1} sm={2} xs={2} sx={{ marginTop: "6px" }}>
                                    ...
                                    <div className={classes.closeicon}>
                                        ...
                                        <Grid item md={12} sm={12} xs={12}>
                                            <Grid style={{ marginLeft: '3em', "&:hover": { backgroundColor: 'red' } }}>
                                                <img src={noimage} onClick={(e) => { fetchProductsall(); }} style={{ width: '110px', height: '130px', cursor: 'pointer' }} />
			...
			{isQuotationTrue && (
				<Button disableRipple sx={userStyle.btnPause} type="submit" onClick={handleSubmitquotation}>
		                    <EditOutlinedIcon style={{ fontSize: "large" }} />Quotation
		                </Button>
			)}
