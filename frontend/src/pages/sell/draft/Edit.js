...
const Draftedit = () => {
    ...
    const { isUserRoleAccess, allProducts } = useContext(UserRoleAccessContext);
    ...
    const taxrateRequest = async () => {
        try {
            ...
            let taxRateData = response.data.taxrates.filter((data) => {
                return data.taxtype == "taxrate" && data.assignbusinessid == setngs.businessid
            })
    ...
    const fetchHandleStock = async () => {
        try {
            ...
            let allstockData = response.data.stocks.map((value) => {
    ...
    const fetchCustomer = async (cusledgeid)=>{
        try {
            let res = await axios.get(`${SERVICE.CUSTOMER_SINGLE}/${cusledgeid}`, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
            });
    ...
    const sendRequestCredit = async () => {
        ...
        await axios.put(`${SERVICE.CUSTOMER_SINGLE}/${posAdd.customerledgeid}`, {
            headers: {
                'Authorization': `Bearer ${auth.APIToken}`
            },
