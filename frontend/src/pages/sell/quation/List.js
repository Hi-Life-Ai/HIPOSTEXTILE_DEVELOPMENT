...
const Quotationlisttable = () => {
    ...
    const fetchQuotation = async () => {
        try {
            let response = await axios.post(SERVICE.ALLQUOTATION, {
                headers: {
    ...
    const getexcelDatas = async () => {
        var data = quotations.map((t, index) => ({
            "Sno": index + 1, "Date": moment(t.date).utc().format('DD-MM-YYYY'), "Quotation No": t.referenceno, 'Location': t.businesslocation, "Customer Name": t.customer, "Contact Number": t.contactnumber,
    ...
    const addSerialNumber = () => {
      const itemsWithSerialNumber = quotations?.map((item, index) => ({ ...item, serialNumber: index + 1 }));
