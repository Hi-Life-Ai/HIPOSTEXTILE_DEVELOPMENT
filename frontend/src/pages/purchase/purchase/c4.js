...
const Purchasecreatelist = () => {
    ...
    const productInputs = {
        ...
        quantityunit: "Pieces",quantitynitpiece:"",quantitysubunitpieces:1,quantitytotalpieces: 1,quantityunitstatus: false,freeitem: "",freeitemunit:"Pieces",freeitemtotalpieces:1,
                    ...
                    if (value.purchasetabletax == "None") {
                        ...
                        if (regex.test(inputValue)) {
                            if (value.sellingpricetax == "Exclusive" || value.sellingpricetax == "Inclusive") {
                                    ...
                                    let unitcost = (Number(netCost) / Number(resquantity == 0 ? 1 : resquantity));
                                    let unitcostdisc = value.enteramt;
                                    //discount
                                    if (value.distypemod == "None") {
                                        ...
                                        //before disc netcost && unit 
                                        let netCostBeforedisc = Number(value.enteramt) * Number(resquantity == 0 ? 1 : resquantity)
                                        let unitcostbrfdisc = (Number(netCostBeforedisc) / Number(resquantity == 0 ? 1 : resquantity));
                                        //after disc netcost
                                        let netCostAftdisc = Number(discountValue) * Number(resquantity == 0 ? 1 : resquantity)
                                        let unitcostaftdisc = (Number(netCostAftdisc) / Number(resquantity == 0 ? 1 : resquantity));
                        ...
                        const regex = /^[0-9]+$/;
                            if (regex.test(value.enteramt)) {
                                if (value.purchasetax == "None" && value.sellingpricetax == "Exclusive" || value.sellingpricetax == "Inclusive") {
                                        ...
                                        let unitcostBefrTax = (Number(netCostBefrTax) / Number(resquantity == 0 ? 1 : resquantity));
                                        let netCostAftTax = Number(purchaseEx) * Number(resquantity == 0 ? 1 : resquantity)
                                        let unitcostAftTax = (Number(netCostAftTax) / Number(resquantity == 0 ? 1 : resquantity));
                                        let netatxamt = Number(netCostBefrTax) - Number(netCostAftTax);
                                        let unittaxamt = Number(unitcostBefrTax) - Number(unitcostAftTax);
                                        //discount
                                        if (value.distypemod == "None") {
                                            ...
                                            //unit nad net cost before discount
                                            let netCostBeforedisc = Number(value.enteramt) * Number(resquantity == 0 ? 1 : resquantity)
                                            let unitCostBeforedisc = (Number(netCostBeforedisc) / Number(resquantity == 0 ? 1 : resquantity));

                                            //unit and net cost after discount
                                            let netCostAfterdisc = Number(discountValue) * Number(resquantity == 0 ? 1 : resquantity)
                                            let unitCostAfterdisc = (Number(netCostAfterdisc) / Number(resquantity == 0 ? 1 : resquantity));

                                            //unit and net cost before disc with tax
                                            let netCostBeforeDisTax = Number(purchaseDiscBfrEx) * Number(resquantity == 0 ? 1 : resquantity)
                                            let unitCostBeforeDisTax = (Number(netCostBeforeDisTax) / Number(resquantity == 0 ? 1 : resquantity));

                                            //unit and net cost after disc with tax
                                            let netCostAfterDisTax = Number(purchaseDiscAftEx) * Number(resquantity == 0 ? 1 : resquantity)
                                            let unitCostAfterDisTax = (Number(netCostAfterDisTax) / Number(resquantity == 0 ? 1 : resquantity));
                                            ...
                                            //unit and net cost after disc with tax
                                            let netCostAfterDisTax = Number(purchaseDiscAftIn) * Number(resquantity == 0 ? 1 : resquantity)
                                            let unitCostAfterDisTax = (Number(netCostAfterDisTax) / Number(resquantity == 0 ? 1 : resquantity));
                        ...
                        if (regex.test(value.enteramt)) {
                            if (value.purchasetax == "None" && value.sellingpricetax == "Exclusive" || value.sellingpricetax == "Inclusive") {
                                ...
                                //quantity
                                let netCostBefrDisc = Number(purchaseEx) * Number(resquantity == 0 ? 1 : resquantity)
                                let unitcostBefrDisc = (Number(netCostBefrDisc) / Number(resquantity == 0 ? 1 : resquantity));
                                let netCostAftDisc = Number(purchaseEx) * Number(resquantity == 0 ? 1 : resquantity)
                                let unitcostAftdisc = (Number(netCostAftDisc) / Number(resquantity == 0 ? 1 : resquantity));

                                let netCostBefrTax = Number(purchaseEx) * Number(resquantity == 0 ? 1 : resquantity)
                                let unitcostBefrTax = (Number(netCostBefrTax) / Number(resquantity == 0 ? 1 : resquantity));
                                let netCostAftTax = Number(purchaseIn) * Number(resquantity == 0 ? 1 : resquantity)
                                let unitcostAftTax = (Number(netCostAftTax) / Number(resquantity == 0 ? 1 : resquantity));
                                let netatxamt = Number(netCostAftTax) - Number(netCostBefrTax);
                                let unittaxamt = Number(unitcostAftTax) - Number(unitcostBefrTax);
                                //discount
                                if (value.distypemod == "None") {
                                    ...
                                    //unit and net cost after discount
                                    let netCostAfterdisc = Number(purchaseDiscAftEx) * Number(resquantity == 0 ? 1 : resquantity)
                                    let unitCostAfterdisc = (Number(netCostAfterdisc) / Number(resquantity == 0 ? 1 : resquantity));
