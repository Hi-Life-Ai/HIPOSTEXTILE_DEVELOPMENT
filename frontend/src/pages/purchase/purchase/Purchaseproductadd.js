...
import { Box, Grid, FormControl, Tooltip, InputLabel, Dialog, DialogTitle, DialogContent, DialogActions, OutlinedInput, TextareaAutosize, Typography, TextField, FormGroup, FormControlLabel, Checkbox, Button, IconButton } from '@mui/material';
...
import Webcamimage from '../../products/Webcamproduct';
import { toast } from 'react-toastify';
import { SERVICE } from '../../../services/Baseservice';
import { AuthContext } from '../../../context/Appcontext';
import Createunitmod from '../../products/product/Createunitmod';
import Createsizemod from '../../products/product/Createsizemod';
import Createcolormod from '../../products/product/Createcolormod';
import Createcategory from '../../products/product/Createcategory';
import Createbrand from '../../products/product/Createbrand';
import Createrack from '../../products/product/Createrack';
import Createstyle from '../../products/product/Createstyles';
import { MultiSelect } from "react-multi-select-component";

function Purchaseproductadd({ setFetchsaveproduct }) {

    //  Modal
    const [productMod, setProductMod] = useState(false);
    const purchaseProModOpen = () => { setProductMod(true); };
    const purchaseProModClose = () => {
        setProduct({
    ...
    const [product, setProduct] = useState({
        ...
        businesslocation: "", managestock: true, minquantity: "", maxquantity: "", rack: "Please Select Rack",
    ...
    const fetchProducts = async () => {
        try {
            ...
            let resultloc = res?.data?.products.map((data, index) => {
                return data.sku
            })
    ...
    const fetchProductsName = async () => {
        try {
            ...
            setCmpProdName(res?.data?.result)
        } catch (err) {
            const messages = err?.response?.data?.message;
            if (messages) {
                toast.error(messages);
    ...
    useEffect(
        () => {
            fetchColor();
        }, [fetchsavecolor, isColorMulti, isMultiColor, colors]
    );
    useEffect(
        () => {
            fetchUnit();
    ...
    useEffect(
        () => {
            fetchSize();
        }, [fetchsavesize, isSizeMulti, isMultiSize, sizes]
    );
    ...
    return (
        <Box>
            <Box>
                <AddCircleOutlineOutlinedIcon onClick={purchaseProModOpen} />
                <Dialog PaperProps={{ style: { borderRadius: "10px" } }}
                    onClose={purchaseProModClose}
                    open={productMod}
                    keepMounted
                    sx={{
                ...
                >
                    <DialogTitle id="customized-dialog-title1" onClose={purchaseProModClose} sx={{ backgroundColor: '#e0e0e0', color: "#000", display: "flex" }}>
                        Add Product
                    </DialogTitle>
                    <DialogContent dividers sx={{
                        minWidth: '800px', height: '850px', '& .MuiOutlinedInput-notchedOutline': {
                    ...
                    <DialogActions>
                        ...
                        <Button sx={userStyle.buttoncancel} autoFocus onClick={purchaseProModClose} disableRipple>CLOSE</Button>
                    </DialogActions>
                </Dialog>
            </Box>
...
export default Purchaseproductadd;