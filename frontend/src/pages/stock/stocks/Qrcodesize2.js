  ...
  return (
    <>
      ...
      <Grid xs={12} md={12} lg={12} sx={12}>
        {productLabel.isProductLocation ? <p className="BusinessLocation" style={{fontSize:'15px',color:"black" ,fontWeight:1200,textAlign:'center'}}><b>{bnname}</b></p> : <p className="BusinessLocation" style={{fontSize:'15px',color:"black" ,fontWeight:1200,textAlign:'center', visibility:'hidden'}}><b>{bnname}</b></p>}

          </Grid>
      <Grid container>
            <Grid xs={2} md={2} lg={2} sm={2} sx={{textAlign:'center'}}>
           {productLabel.isProductNumberAlpha ? <p style = {{marginTop:'15px',fontSize:resnodata == 2 ? '14px': resnodata == 3 ? '13px' : resnodata == 4 ? '12px' : resnodata == 5 ? '11px' : '15px',marginLeft:'-14px',fontWeight:1200,transform:'rotate(-90deg)',color:'black',position:'absolute',textAlign:'center',textTransform:'uppercase'}}><b>{getProductData.snno + "#" + getProductData.alpharate}</b></p> : <p style = {{marginTop:'15px',fontSize:resnodata == 2 ? '14px': resnodata == 3 ? '13px' : resnodata == 4 ? '12px' : resnodata == 5 ? '11px' : '15px',marginLeft:'-14px',fontWeight:1200,transform:'rotate(-90deg)',color:'black',position:'absolute',textAlign:'center',textTransform:'uppercase', visibility:'hidden'}}><b>{getProductData.snno + "#" + getProductData.alpharate}</b></p>} 

            </Grid>
            <Grid xs={1} md={1} lg={1} sm={1} sx={{textAlign:'center'}}>
            <Typography>{productLabel.isProductCode ? <p style = {{fontSize:'12px',marginTop:'45px',marginLeft:'-2px', transform:'rotate(-90deg)',fontWeight:'bolder',textAlign:'center',color:'black',textTransform:'uppercase'}}><b>{rackslice + '/' + skuid }</b></p> : <p style = {{fontSize:'11px',marginTop:'45px',marginLeft:'-2px', transform:'rotate(-90deg)',fontWeight:'bolder',textAlign:'center',color:'black',textTransform:'uppercase', visibility:'hidden'}}><b>{rackslice + '/' + skuid }</b></p>}</Typography>
