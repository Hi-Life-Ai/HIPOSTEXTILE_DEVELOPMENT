...
const StockAdjustmentlist = () => {
    ...
    const handleCheckboxChange = (productname, isChecked) => {
        setSelectedProduct(isChecked ? productname : null);
        setPagePro(parseInt(1));
        
    };

    // Reorder the filteredDataPro array based on the selected product
    const reorderedData = selectedProduct
        ? [filteredDataPro.find((row) => row.productname === selectedProduct), ...filteredDataPro.filter((row) => row.productname !== selectedProduct)]
        : filteredDataPro;
    ...
    const fetchCategory = async () => {
        try {
            ...
            setCategory(res?.data?.categories.map((d) => (
                {
                    ...d,
    ...
    const fetchBrand = async () => {
        try {
            ...
            setBrand(res?.data?.brands.map((d) => (
                {
                    ...d,
                    label: d.brandname,
                    value: d.brandname
    ...
    const fetchColor = async () => {
        try {
            ...
            setColor(res?.data?.colors.map((d) => (
                {
                    ...d,
                    label: d.colorname,
                    value: d.colorname
    ...
    const fetchStyle = async () => {
        try {
            ...
            setStyle(res?.data?.styles.map((d) => (
                {
                    ...d,
                    label: d.stylename,
                    value: d.stylename
    ...
    const handleChangeProductPurchase = async (skuid, productnamevalue, currentstockvalue) =>{
        ...
        try {
            var res = await axios.post(SERVICE.ADJUSTMENTPRODUCTPURCHASE, {
                headers: {
                    'Authorization': `Bearer ${auth.APIToken}`
                },
                businessid: String(setngs.businessid),
            ...
            setIsProductsPurchase(res?.data?.purchasesproducts);
        } catch (err) {
            const messages = err?.response?.data?.message;
            if (messages) {
                toast.error(messages);
                                            ...
                                            <Grid>
                                                <FormGroup>
                                                    <FormControlLabel
                                                        control={
                                                            <Checkbox
                                                                checked={selectedProduct === row?.productname}
                                                                onChange={(e) => {handleCheckboxChange(row?.productname, e.target.checked); handleChangeProductPurchase(row?.sku, row?.productname, row?.currentstock);}}
                                                                disabled={selectedProduct && selectedProduct !== row?.productname}
                                                            />
                                                        }
                                                    />
                                                </FormGroup>
